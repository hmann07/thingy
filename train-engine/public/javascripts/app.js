...
svg.append('svg:defs').append('svg:marker')
  ...
  var linkWidthScale = d3.scaleLinear().range([2,7]).domain([d3.min(outputlinks, function(d){return Math.abs(d.weight)}), d3.max(outputlinks,function(d){return Math.abs(d.weight)})]);
  ...
  var color = d3.scaleLinear().range(["#5DB3F1", "#BDDBF1", "#F1D8DA", "#F16069"]).domain([-4,-0.001,0.001,4]);
  var link = svg.selectAll(".link")
    .data(links)
    .enter().append("path")
      .attr("class", "link")
      .style("stroke-width", function(d){ return linkWidthScale(Math.abs(d.weight)) + "px"})
